...
extern unsigned char i_JDI_CTPM_FW[];
extern unsigned char i_LEAD_CTPM_FW[];
#endif
...
int himax_palm_detect(uint8_t *buf)
{
	...
	if(buf[PALM_BYTE]!=255 && (buf[PALM_BYTE] & 1<<PALM_BIT)>0){
		I("%s: detect\n",__func__);
		return NO_ERR;
	}
	else{
...
// Product ID
        ...
        if ((tmp_data[3] == 0x83) && (tmp_data[2] == 0x11) && ((tmp_data[1] == 0x2a)||(tmp_data[1] == 0x2b)))
        {
			...
			if(nubia_panel_id == 1)
			{
				g_i_FW_VER = i_LEAD_CTPM_FW[FW_VER_MAJ_FLASH_ADDR]<<8 |i_LEAD_CTPM_FW[FW_VER_MIN_FLASH_ADDR];
				g_i_CFG_VER = i_LEAD_CTPM_FW[CFG_VER_MAJ_FLASH_ADDR]<<8 |i_LEAD_CTPM_FW[CFG_VER_MIN_FLASH_ADDR];
				g_i_CID_MAJ = i_LEAD_CTPM_FW[CID_VER_MAJ_FLASH_ADDR];
				g_i_CID_MIN = i_LEAD_CTPM_FW[CID_VER_MIN_FLASH_ADDR];
			}
			else if(nubia_panel_id == 0)
			{
				g_i_FW_VER = i_JDI_CTPM_FW[FW_VER_MAJ_FLASH_ADDR]<<8 |i_JDI_CTPM_FW[FW_VER_MIN_FLASH_ADDR];
				g_i_CFG_VER = i_JDI_CTPM_FW[CFG_VER_MAJ_FLASH_ADDR]<<8 |i_JDI_CTPM_FW[CFG_VER_MIN_FLASH_ADDR];
				g_i_CID_MAJ = i_JDI_CTPM_FW[CID_VER_MAJ_FLASH_ADDR];
				g_i_CID_MIN = i_JDI_CTPM_FW[CID_VER_MIN_FLASH_ADDR];
			}
#else
			g_i_FW_VER = i_JDI_CTPM_FW[FW_VER_MAJ_FLASH_ADDR]<<8 |i_JDI_CTPM_FW[FW_VER_MIN_FLASH_ADDR];
			g_i_CFG_VER = i_JDI_CTPM_FW[CFG_VER_MAJ_FLASH_ADDR]<<8 |i_JDI_CTPM_FW[CFG_VER_MIN_FLASH_ADDR];
