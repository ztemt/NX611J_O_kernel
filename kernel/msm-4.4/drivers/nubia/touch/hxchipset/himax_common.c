...
#if defined (NUBIA_DISP_LCD_PANEL_ID)
#include "../../display/nubia_lcd_feature.h"
#endif

#ifdef CONFIG_NUBIA_TP_HIMAX_ESD_CHECK
...
#endif
#ifdef HX_USB_DETECT_GLOBAL
static int himax_power_notifier_cb(struct notifier_block *self,
...
#endif
#if defined (NUBIA_TP_HIMAX_PALM_REPORT)
...
static int i_update_FW(void)
{
	...
	if(nubia_panel_id == 1)
	{
		ImageBuffer = i_LEAD_CTPM_FW;
		fullFileLength = sizeof(i_LEAD_CTPM_FW);
	}
...
#if defined(HX_PALM_REPORT)
#if defined(NUBIA_TP_HIMAX_PALM_REPORT)
        if(himax_palm_detect(hx_touch_data->hx_coord_buf) == NO_ERR && ts->palm_enable)
        {
            I(" %s HX_PALM_REPORT KEY power event press\n",__func__);
            input_report_key(ts->input_dev, KEY_F9, 1);
            input_sync(ts->input_dev);
            input_report_key(ts->input_dev, KEY_F9, 0);
            input_sync(ts->input_dev);
            return;
        }
#else
...
#ifdef CONFIG_NUBIA_TP_HIMAX_ESD_CHECK
            if(hx_EB_event_flag > 5
                ||hx_EC_event_flag > 5
                ||hx_ED_event_flag > 5){
                //esd lcm reset
                E("nubia_fb_report_panel_dead-- hx_EB_event_flag=%d hx_EC_event_flag=%d hx_ED_event_flag=%d", hx_EB_event_flag, hx_EC_event_flag, hx_ED_event_flag);
                nubia_fb_report_panel_dead();
                hx_EB_event_flag=0;
                hx_EC_event_flag=0;
                hx_ED_event_flag=0;
            }
		...
		unsigned long val, void *v)
{
	...
	if (IS_ERR(psy->desc->get_property) || psy->desc->get_property == NULL) {
		I("%s:%d  psy  get_property  is  NULL \n",__func__,__LINE__);
		return NOTIFY_OK;
	}

	if (psy->desc->get_property != NULL ) {
		retval = psy->desc->get_property(psy,POWER_SUPPLY_PROP_STATUS,&prop);
	}
	if (retval != 0) {
		return NOTIFY_OK;
	}
	...
	if (status != POWER_SUPPLY_STATUS_DISCHARGING && last_status == POWER_SUPPLY_STATUS_DISCHARGING) {
		...
		last_status = status;
		USB_detect_flag = 0;
	} else if( status == POWER_SUPPLY_STATUS_CHARGING && last_status == POWER_SUPPLY_STATUS_UNKNOWN){
		last_status = status;
		USB_detect_flag = 1;
	}else {
		last_status = status;
...
#ifdef HX_USB_DETECT_GLOBAL
    ts->power_notifier.notifier_call = himax_power_notifier_cb;
    if(power_supply_reg_notifier(&(ts->power_notifier)) < 0){
        I("%s: Failed to register power notifier client\n",	__func__);
    }
#endif
...
int himax_chip_common_suspend(struct himax_ts_data *ts)
{
    ...
    if(nubia_get_diag_command() > 0){
        nubia_close_diag_debug();
    }
#ifdef HX_TP_PROC_FLASH_DUMP
