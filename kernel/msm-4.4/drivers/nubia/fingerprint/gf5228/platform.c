...
int gf_parse_dts(struct gf_dev* gf_dev)
{
	int rc = 0;
	/*get reset resource*/
	gf_dev->pwr_gpio_vdd = of_get_named_gpio(gf_dev->spi->dev.of_node,"goodix,goodix_pwr_vdd",0);
	if(!gpio_is_valid(gf_dev->pwr_gpio_vdd)) {
		FP_LOG_ERROR("POWER GPIO is invalid.\n");
		return -1;
	}
	...
	gf_dev->pwr_gpio_vddio = of_get_named_gpio(gf_dev->spi->dev.of_node,"goodix,goodix_pwr_vddio",0);
	if(!gpio_is_valid(gf_dev->pwr_gpio_vddio)) {
		FP_LOG_ERROR("pwr_vddio GPIO is invalid.\n");
		return -1;
	}
	...
	/*get reset resource*/
	gf_dev->reset_gpio = of_get_named_gpio(gf_dev->spi->dev.of_node,"goodix,goodix_reset",0);
	...
	/*get irq resourece*/
/*	
	gf_dev->irq_gpio = of_get_named_gpio(gf_dev->spi->dev.of_node,"goodix,goodix_irq",0);
...
*/	
	...
	rc = gpio_request(gf_dev->irq_gpio, "goodix_irq");
	FP_LOG_ERROR("request IRQ GPIO. rc = %d\n", rc);
