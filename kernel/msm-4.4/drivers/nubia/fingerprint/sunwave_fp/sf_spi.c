...
static struct clk *sf_clk_get(struct device_node *dev_node, const char *con_id)
{
    const char *dev_id = NULL;
    struct clk *clk;

    if (dev_node) {
        clk = of_clk_get_by_name(dev_node, con_id);

        if (!IS_ERR(clk)) {
            xprintk(KERN_DEBUG, "%s(..) 111. is here good !!!.\n", __FUNCTION__);
            return clk;
        }
    ...
    xprintk(KERN_DEBUG, "%s(..) 222. is here ba ga !!!.\n", __FUNCTION__);
...
int sf_spi_clock_init(void)
{
    ...
    clk_enabled = 0;
    core_clk = sf_clk_get(dev_node, "core_clk");
    ...
    iface_clk = sf_clk_get(dev_node, "iface_clk");
