...
static int pa224_get_psoffset(struct i2c_client *client)
{
	...
	if (tmp_uncover_data < PA24_PS_UNCOVER_MIN_SAFE || tmp_uncover_data > PA24_PS_UNCOVER_MAX_SAFE) {
...
static void pa224_report_event(struct pa224_data *data)
{
	if (data->ps_status != data->ps_last_status) {
		...
		SENSOR_LOG_INFO("data->ps_status = %d\n", data->ps_status);
	}
	return;
}
/*
		...
		struct device_attribute *attr,	char *buf)
{
	...
	return sprintf(buf, "prox debug %s: crosstalk = %d near_diff_cnt = %d\n",
     data->debug_level ? "on":"off", data->crosstalk, data->near_diff_cnt);
}
static ssize_t pa224_prox_uncover_data_min_show(struct device *dev,
		struct device_attribute *attr,	char *buf)
{
...
static struct device_attribute attrs_prox_device[] = {
	...
	__ATTR(prox_uncover_min, 0444, pa224_prox_uncover_data_min_show, NULL),
	...
	__ATTR(debug, 0444, pa224_cal_debug_show, NULL),
};
