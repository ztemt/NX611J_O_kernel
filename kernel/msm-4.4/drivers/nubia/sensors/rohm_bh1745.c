 ...
 * RETURN     : Lux value
 *****************************************************************************/
static int rgb_bh1745_calc_lx(struct i2c_client *client, struct rgb_bh1745_data *rdata, unsigned char gain, unsigned short itime)
{
	long long int lx;
...
static int rgb_bh1745_get_lux(struct rgb_bh1745_data *data, bool report_event)
{
	...
	if (data->debug_level) {
		SENSOR_LOG_INFO("rgb bh1745 data->rgb_data.red(%d); data->rgb_data.green(%d);data->rgb_data.blue(%d);data->rgb_data.clear(%d)\n",
			data->rgb_data.red,
			data->rgb_data.green,
			data->rgb_data.blue,
			data->rgb_data.clear);
		...
		|| (data->rgb_data.clear < 0))
	{
		...
		luxValue = rgb_bh1745_calc_lx(client, data, gain, time);
	}
	if (luxValue >= 0) {
	    ret = 0;
...
get_lux_exit:
	...
	if (lux_is_valid && report_event) {
		if (data->rgb_cal_data.flag) {
		    ...
		    if (data->debug_level)
		        SENSOR_LOG_INFO("rgb bh1745 cal lux=%d\n",data->als_cal_lux);
		} else {
		    input_report_rel(data->input_dev_als, REL_X, data->als_prev_lux + 1);
		    if (data->debug_level)
		        SENSOR_LOG_INFO("rgb bh1745 lux=%d\n",data->als_prev_lux);
		}
...
static ssize_t rgb_bh1745_calibrate_work(struct rgb_bh1745_data *data, const char *cal_data)
{
	...
	if (data->rgb_cal_data.base.lux / data->rgb_cal_data.cur.lux > 4) {
	    err = -EINVAL;
	    data->rgb_cal_data.flag = 0;
...
static ssize_t attr_rgb_flush_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int count;
	struct rgb_bh1745_data *data = dev_get_drvdata(dev);
	if (data->rgb_cal_data.flag) {
	...
	SENSOR_LOG_INFO("flush: als_cal_lux = %d ,als_lux =%d\n",data->als_cal_lux, data->als_prev_lux);
	return count;
}
static ssize_t attr_rgb_lux_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (data->enable_als_sensor) {
	    ...
	    SENSOR_LOG_INFO("enabled, waiting for updated data\n");
	} else {
	    SENSOR_LOG_INFO("disabled, need enable and waiting for updated data\n");
	    ret = rgb_bh1745_enable_prepare(data);
	    if (ret < 0) {
	        SENSOR_LOG_ERROR("enable rgb failed\n");
	        return ret;
	    }
	    msleep(MEASURE_DELAY_320MS);
	    while(retry_times) {
	        ret = rgb_bh1745_get_lux(data, true);
	        if (ret == 0) {
	            SENSOR_LOG_ERROR("read lux ok\n");
	            break;
	        }
	        retry_times--;
	        msleep(5);
...
static ssize_t attr_lux_debug_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct rgb_bh1745_data *data = dev_get_drvdata(dev);
	data->debug_level = !data->debug_level;
	return sprintf(buf, "%x\n", data->debug_level);
}
static ssize_t attr_lux_debug_store(struct device *dev,
                   struct device_attribute *attr, const char *buf, size_t count)
{
	int err;
	int val = 1;
	...
	data->debug_level = (unsigned int) val;
	return count;
}
/*
