...
#include "msm_cci.h"
#include "../../../../../../video/fbdev/msm/mdss_fb.h" //ZTEMT: added by congshan for front camera flash
	...
	struct msm_flash_ctrl_t *fctrl)
{
	...
	if (fctrl->flash_driver_type == FLASH_DRIVER_DEFAULT)
		fctrl->flash_driver_type = FLASH_DRIVER_PMIC;//ZTEMT: guxiaodong modify for pmic flash
	CDBG("%s:%d fctrl->flash_driver_type = %d", __func__, __LINE__,
		fctrl->flash_driver_type);
...

{
	...
	bkl_recov = val;
	if((0 != bkl_recov) && (0 != val)) {
		zte_camera_backlight(zte_camera_mfd, CFG_LCD_BKL_NORM);
	}
	return size;
}
